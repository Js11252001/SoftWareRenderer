...
void window_draw(TGAImage& image)
{
	int i, j;
	for (int i = 0; i < image.get_height(); i++)
	{
		for (int j = 0; j < image.get_width(); j++)
		{
			...
			window->window_fb[index] = image.get(j,i).bgra[0];
			window->window_fb[index + 1] = image.get(j, i).bgra[1];
			window->window_fb[index + 2] = image.get(j, i).bgra[2];
		}
	}
	window_display();
}
